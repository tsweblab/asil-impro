...
export async function getSettings() {
  ...
  return {
    ...
    evenementBouton: settings?.evenementBouton ?? 'Réserver',
    atelierLieuLabel: settings?.atelierLieuLabel ?? 'Lieu :',
    atelierTarifLabel: settings?.atelierTarifLabel ?? 'Tarif annuel :',
    stageHorairesLabel: settings?.stageHorairesLabel ?? 'Horaires :',
    stageLieuLabel: settings?.stageLieuLabel ?? 'Lieu :',
    stageIntervenantLabel: settings?.stageIntervenantLabel ?? 'Intervenant·e :',
    stageTarifLabel: settings?.stageTarifLabel ?? 'Tarif :',
    stageBouton: settings?.stageBouton ?? "S'inscrire",
    stageDuLabel: settings?.stageDuLabel ?? 'Du',
    stageAuLabel: settings?.stageAuLabel ?? 'au',
    stageImageFallback: settings?.stageImageFallback ?? 'ASIL',
    ...
    photoRejoindreAlt: settings?.photoRejoindreAlt ?? "Improvisatrice de l'ASIL sur scène",
    photoAsil: settings?.photoAsil ?? null,
    photoAsilAlt: settings?.photoAsilAlt ?? "Comédiens de l'ASIL en spectacle d'improvisation",
    photoAteliers: settings?.photoAteliers ?? null,
    photoAteliersAlt: settings?.photoAteliersAlt ?? "Atelier d'improvisation de l'ASIL",
    photoStages: settings?.photoStages ?? null,
    photoStagesAlt: settings?.photoStagesAlt ?? "Participants d'un stage d'improvisation de l'ASIL",
    photoContact: settings?.photoContact ?? null,
    photoContactAlt: settings?.photoContactAlt ?? "Spectacle d'improvisation de l'ASIL",
  };
}
